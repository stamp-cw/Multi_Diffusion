...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            # Generate Poisson noise with mean 100
            poisson_noise = torch.poisson(torch.full_like(x_start, 100.0))
            # Normalize to have zero mean and unit variance (similar to standard normal)
            noise = (poisson_noise - 100.0) / 10.0  # Since Var(Poisson(λ)) = λ, std = √λ = 10
