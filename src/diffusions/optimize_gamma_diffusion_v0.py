...
from src.utils import linear_beta_schedule, cosine_beta_schedule, sigmoid_beta_schedule, sqrt_beta_schedule, \
    optimize_gamma_linear_beta_schedule
import torch.nn.functional as F


####################################################################################################
# OptimizeGammaDiffusion
####################################################################################################
class OGammaDiffusion:
    def __init__(
            self,
            timesteps=1000,
            theta_0=0.00001,
        ...
        if beta_schedule == 'linear':
            betas = optimize_gamma_linear_beta_schedule(timesteps)
    ...
    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            ...
            noise = Gamma(kappas_cumsum_t.squeeze(), (1 / thetas_t).squeeze()).sample(x_start.shape[1:]).permute(
                [-1, 0, 1, 2])
